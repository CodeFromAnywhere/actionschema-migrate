...
export const upsertCrudOpenapis = async (context) => {
    ...
    const remoteSchemas = remoteCrudSchemaUrls
        ? (await Promise.all(remoteCrudSchemaUrls.map(async (url) => {
    ...
    const results = await Promise.all(fileSchemas.map(async (item) => {
    ...
    const changedEnvironmentVariables = mergeObjectsArray(changedEnvironmentVariablesArray);
    addOrReplaceEnvKeys(path.join(process.cwd(), ".env"), changedEnvironmentVariables);
